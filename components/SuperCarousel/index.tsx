...
export interface SuperCarouselProps {
  title: string;
    ...
    <>
      <style>
        {`.group\\/${title} {
          ...
          --card-height: calc(((100vw - var(--padding-size)*2 - var(--gap-size)*${
            cardNumber - 1
          }) / var(--card-number)*1)/0.66667);
          --card-expanded-width: calc(var(--card-height)*1.77778);
        }`}
      </style>
      <div ref={containerRef} className={`group/${title}`}>
        <section className="mb-24 box-border">
          ...
          <div
            className="group/sliderWrapper grid grid-cols-[repeat(2,calc(var(--padding-size)-var(--gap-size)))] grid-rows-[2fr] justify-between overflow-hidden group-hover/slider:mb-[-500px] group-hover/slider:mt-[-150px]"
            style={{
          ...
          >
            <button
              ...
              style={{ gridArea: 'left', height: `var(--card-height)` }}
            >
              <IoChevronBackSharp />
            </button>
            <ul
              className="col-[left-start/right-end] row-[1] flex translate-x-[calc(var(--slider-scroll)*-100%)] flex-row items-stretch overflow-x-scroll scroll-smooth p-[0_var(--padding-size)] transition-transform duration-300 group-hover/slider:z-10 group-hover/slider:pb-[500px] group-hover/slider:pt-[150px] webkitScrollbar:hidden"
              style={
                {
                  '--slider-scroll': 0,
            ...
            <button
              ...
              style={{ gridArea: 'right', height: 'var(--card-height)' }}
            >
